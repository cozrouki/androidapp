...
public class ToastNotification : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    ...
    public static float minimumMessageTime = 3; // 모든 메시지가 표시되는 최소 시간입니다.
    public static bool hideOnClick = true; // 클릭하면 메시지를 숨기거나 닫습니다.
    ...
    [Header("기본 메시지 패턴:")]
    [Tooltip("메시지에 타이머로 카운트다운 이미지가 표시됩니다.")]
    public bool _showTimerRender = true;
    [Tooltip("메시지에 기본 Light 테마를 사용하려면 비활성화하세요.")]
    public bool _darkTheme = true;
    [Tooltip("모든 메시지가 표시되는 최소 시간입니다.")]
    public float _minimumMessageTime = 3;
    [Tooltip("모서리에 X 여백과 Y 여백이 있습니다. X 여백은 중앙 집중식 메시지에서는 작동하지 않습니다.")]
    public Vector2 _margin = new Vector2(20, 20);
    [Tooltip("마우스 커서가 ToastNotification 객체 위에 있을 때 타이머를 중지합니다.")]
    public bool _stopOnOver = true;
    [Tooltip("클릭하면 메시지를 숨기거나 닫습니다.")]
    public bool _hideOnClick = true;
    [Tooltip("화면상의 메시지 위치")]
    public MessageScreenPosition _messageScreenPosition = MessageScreenPosition.TopRight;
    [Tooltip("타이머 카운트다운 방향입니다. 자동으로 메시지 화면 위치 옵션을 기준으로 최적의 위치를 ​​선택합니다.")]
